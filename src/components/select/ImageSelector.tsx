...
import {loadImageNames, prefixIfOnMacOS, rotateImage} from "../../util/functions";
...
export const ImageSelector: FC = () => {
  ...
  useEffect(() => {
    ...
    getColorFromURL(`${prefixIfOnMacOS()}${fromPath.current}`).then(color => {
  ...
  return (
    ...
    >
      {exists && <CheckIcon />}
      <img
        src={`${prefixIfOnMacOS()}${imageToDisplay.src}?${imageToDisplay.timestamp}`}
